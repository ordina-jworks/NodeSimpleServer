...
export class SlotmachineEndpoint extends BaseEndpoint {
    ...
    private handleButtonTrigger = (data: any): void => {
        if(data) {
            if((data.payload == true || data.payload == 'true' || data.payload == 1)&& data.container === "push") {
                this.messageManager.sendMessage({buttonPressed: true, deveui:data.DevEUI}, MessageTarget.INTERVAL_WORKER, 'broadcastMessage');
